...
/*
 * Copyright 2011 JSON-SMART authors Licensed under the Apache License, Version
...
public abstract class BeansMapper<T> extends JsonReaderI<T>
{
    ...
    public static class Bean<T> extends JsonReaderI<T>
    {
        ...
        {
            Accessor nfo = index.get(key);
            if (nfo == null) throw new RuntimeException("Can not find Array '" + key + "' field in " + clz);
        ...
        public JsonReaderI<?> startObject(String key)
        {
            ...
            if (f == null) throw new RuntimeException("Can not find Object '" + key + "' field in " + clz);
    ...
    public static class BeanNoConv<T> extends JsonReaderI<T>
    {
        ...
        {
            Accessor nfo = index.get(key);
            if (nfo == null) throw new RuntimeException("Can not set " + key + " field in " + clz);
            return base.getMapper(nfo.getGenericType());
        }
        ...
        {
            Accessor f = index.get(key);
            if (f == null) throw new RuntimeException("Can not set " + key + " field in " + clz);
            return base.getMapper(f.getGenericType());
        }
