...
public class RSA {
	...
	public RSA(String peerPublicKey,String localPrivateKey) {
	...
	public byte[] decrypt(byte[] data)
			throws Exception {
		// 对密钥解密
		byte[] keyBytes = decryptBASE64(this.localPrivateKey);
		// 取得私钥
		PKCS8EncodedKeySpec pkcs8KeySpec = new PKCS8EncodedKeySpec(keyBytes);
		KeyFactory keyFactory = KeyFactory.getInstance(KEY_ALGORITHM);
		Key privateKey = keyFactory.generatePrivate(pkcs8KeySpec);
	...
	public byte[] encrypt(byte[] data)
			throws Exception {
		// 对公钥解密
		byte[] keyBytes = decryptBASE64(this.peerPublicKey);
	...
	public static void main(String[] args) throws Exception {
		...
		String inputStr = "地方大幅度发";
		byte[] data = inputStr.getBytes("utf-8");

		byte[] encodedData = rsa.encrypt(data);
		
		String str = new String(encodedData,"utf-8");
		System.out.println("加密前,明文="+inputStr+",密文="+str);

		byte[] decodedData = rsa.decrypt(encodedData);
